...
//let dataBuffer = fs.readFileSync('./test/data/3721222-40690059.pdf'); // './test/data/3721222-40690059.pdf'
let i = 0;
let sinnombre = 0;


async function comenzar() {
    fs.readdir('./test/data/data', async (err, files) => {
        if (err) {
            console.log(err);
        }
        console.log(files)
...
async function leerpdf(files) {
    ...
    for (const file of files) {
        ...
        let nombrecompleto = ""
        let ubicacion = ""
        let dataBuffer = fs.readFileSync(`./test/data/data/${file}`);
        await pdf(dataBuffer).then(async function (data) {
            dni = buscardni(data);
            nombrecompleto = buscarnombreyapellido(data);
            ubicacion = `./test/data/data/${file}`; //console.log(dni + nombrecompleto + ubicacion)
